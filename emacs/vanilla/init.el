...
;; `file-name-handler-alist` is consulted on every require or load so doing some tweaks here
;; gives some speed up
(unless (daemonp)
...
(hercules-def
 ...
 :hide-funs '(windresize-exit windowresize-cancel-and-quit)
...
"wr" '(windresize :which-key "resize")
...
"sm" '(ace-maximize-window :which-key "Maximize Window")
...
"TAB" '(nil :which-key "Work[s]paces")
"TAB-s" '(persp-switch :which-key "Switch")
"TAB-n" '(persp-switch-by-number :which-key "Swich By #")
...
"TAB-a" '(persp-add-buffer :which-key "Add a buffer")
"TAB-c" '(persp-set-buffer :which-key "Claim buffer for perspective")
"TAB-b" '(persp-switch-to-buffer :which-key "Switch to buffer (in any perspective)")
"TAB-S" '(persp-state-save :which-key "Save")
"TAB-L" '(persp-state-load :which-key "Load")
